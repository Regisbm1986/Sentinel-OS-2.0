...
from sentinel_os.platform.backend.dashboard.operations_dashboard import (
...
def test_dashboard_snapshot_uses_real_project_data(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sentinel_os.platform.backend.agents.remote_worker_manager.RemoteWorkerManager.get_workers",
...
def test_dashboard_loaders_return_expected_sections(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sentinel_os.platform.backend.agents.task_queue.AGENT_TASKS_DIR", tasks_dir)
    monkeypatch.setattr("sentinel_os.platform.backend.agents.remote_worker_manager.RemoteWorkerManager.get_workers", lambda self: [])
    monkeypatch.setattr(WorkerHeartbeat, "is_alive", lambda self, worker_id, timeout=300: False)
    monkeypatch.setattr("sentinel_os.platform.backend.dashboard.operations_dashboard.PROJECT_ROOT", project_root)
