from sentinel_platform.backend.api.control import submit_module_execution
from sentinel_platform.backend.platform.operations_platform import (
...
def test_execute_module_dispatches_to_existing_module_functions(monkeypatch):
    ...
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_nikto_api",
        fake_nikto,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_spiderfoot",
        fake_spiderfoot,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_dagda",
        fake_dagda,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_kube_hunter",
        fake_kubehunter,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_enum4linux",
        fake_enum4linux,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_john_the_ripper",
        fake_john,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_beef_daemon",
        fake_beef,
    )
    monkeypatch.setattr(
        "sentinel_platform.backend.platform.operations_platform.run_setoolkit_daemon",
        fake_set,
    )
...
def test_build_module_execution_task_creates_queueable_command():
    ...
    assert "sentinel_platform.backend.modules.nikto.module" in task["command"]
