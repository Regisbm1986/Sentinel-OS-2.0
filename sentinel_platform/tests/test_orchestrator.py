...
def test_orchestrator_logs_execution_start_and_end(monkeypatch):
    ...
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerDispatcher", FakeDispatcher)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerExecutor", lambda: FakeExecutor({"status": "completed"}))
...
def test_orchestrator_logs_failed_task_status(monkeypatch):
    telemetry = FakeTelemetry()
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerDispatcher", FakeDispatcher)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerExecutor", lambda: FakeExecutor({"status": "failed", "error": "boom"}))
...
def test_orchestrator_handles_executor_exception(monkeypatch):
    telemetry = FakeTelemetry()
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerDispatcher", FakeDispatcher)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkerExecutor", FakeExceptionExecutor)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.TaskHistory", FakeHistory)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.WorkflowStateManager", FakeWorkflow)
    monkeypatch.setattr("sentinel_platform.backend.agents.orchestrator.AgentMemory", FakeMemory)
