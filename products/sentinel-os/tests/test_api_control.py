...
def test_autonomous_status_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("backend.api.main.PROJECT_ROOT", project_root)
...
def test_telemetry_endpoint_returns_logs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "backend.api.main.ExecutionTelemetry",
...
def test_workers_endpoint_returns_available_workers(monkeypatch):
    monkeypatch.setattr(
        "backend.api.main.WorkerSelector.get_available_workers",
...
def test_autonomous_status_and_run_cycle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("backend.api.main.WorkerSelector.get_available_workers", lambda self: ["worker-1"])
...
def test_telemetry_returns_list(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("backend.api.main.ExecutionTelemetry", lambda *args, **kwargs: __import__("sentinel_platform.backend.telemetry.execution_telemetry", fromlist=["ExecutionTelemetry"]).ExecutionTelemetry(log_path=telemetry_path))
