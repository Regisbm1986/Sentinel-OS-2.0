from backend.api.control import submit_module_execution
from backend.platform.operations_platform import (
...
def test_execute_module_dispatches_to_existing_module_functions(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_nikto_api",
        fake_nikto,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_spiderfoot",
        fake_spiderfoot,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_dagda",
        fake_dagda,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_kube_hunter",
        fake_kubehunter,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_enum4linux",
        fake_enum4linux,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_john_the_ripper",
        fake_john,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_beef_daemon",
        fake_beef,
    )
    monkeypatch.setattr(
        "backend.platform.operations_platform.run_setoolkit_daemon",
        fake_set,
    )
